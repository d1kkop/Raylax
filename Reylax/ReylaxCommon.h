...
#define BVH_NUM_FACES_IN_LEAF 8
...
namespace Reylax
{
    ...
    struct BvhNode
    {
        ...
        using SplitFunction = std::function<void (const MeshData* const *, std::vector<Face>& faces, const vec3& bMin, const vec3& bMax, float& s, u32& splitAxis)>;
        using SidesFunction = std::function<void (u32*, const BvhNode*, u32)>;

        static void splitDefault(const MeshData* const *, const std::vector<Face>& faces, const vec3& bMin, const vec3& bMax, float& s, u32& splitAxis);
        static void splitCenter (const MeshData* const *, const std::vector<Face>& faces, const vec3& bMin, const vec3& bMax, float& s, u32& splitAxis);
        static void splitMedian (const MeshData* const *, const std::vector<Face>& faces, const vec3& bMin, const vec3& bMax, float& s, u32& splitAxis);

        static void sidesDefault( u32* sides, const BvhNode* nodes, u32 numLeafs );
        static void sidesBbox( u32* sides, const BvhNode* nodes, u32 numLeafs );
        ...
        static u32 build( const MeshData** meshData, u32 numMeshDatas, 
                          ...
                          vec3& worldMin,
                          vec3& worldMax,
                          SplitFunction splitFunc = splitDefault,
                          SidesFunction sidesFunc = sidesDefault );
    ...
    struct Store
    {
        ...
        u32 m_partMax;
            ...
            u32 m_offsets[RL_NUM_INNER_QUEUES];
        #if RL_CUDA || !RL_CPU_MT
            u32 m_top[RL_NUM_INNER_QUEUES];
        #else
            std::atomic<u32> m_top[RL_NUM_INNER_QUEUES];
        #endif
    #else
        #if RL_CUDA || !RL_CPU_MT
            u32 m_top;
    ...
    #if RL_USE_INNER_QUEUES
            {
                u32 qIdx = idx&(RL_NUM_INNER_QUEUES-1);
        ...
        FDEVICE_DYN u32 getTotalLength()
        {
        #if RL_USE_INNER_QUEUES
            u32 totalLength = 0;
            for (auto & t : m_top) totalLength += t;
            return totalLength;
        #else
            return m_top;
        #endif
...
/*
    ...
    FDEVICE INLINE bool AABBInAABB(const vec3& tMin, const vec3& tMax, const vec3& bMin, const vec3& bMax)
    {
        if ( !(tMin[0] > bMin[0] && tMax[0] < bMax[0]) ) return false;
        if ( !(tMin[1] > bMin[1] && tMax[1] < bMax[1]) ) return false;
        if ( !(tMin[2] > bMin[2] && tMax[2] < bMax[2]) ) return false;
        return true;
    }
